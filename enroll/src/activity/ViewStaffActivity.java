...
public class ViewStaffActivity extends JFrame implements ActionListener {
	...
	JComboBox byWhatCB;
	JTable table;
	private JButton buttonLogout, buttonBack, buttonCheck, buttonAddStaff;
	private JLabel title, header, keywordLabel;
	JTextField keywordTF;
	
	public ViewStaffActivity(JFrame prev, Staff Staff) {
		super("View Staff");
		...
		buttonAddStaff = new JButton("Add");
		buttonAddStaff.setBounds(Theme.GUI_WIDTH-140, 115, Theme.BUTTON_PRIMARY_WIDTH, 30);
		buttonAddStaff.setFont(Theme.FONT_BUTTON);
		buttonAddStaff.setBackground(Theme.BACKGROUND_BUTTON_PRIMARY);
		buttonAddStaff.setForeground(Theme.COLOR_BUTTON_PRIMARY);
		buttonAddStaff.addActionListener(this);
		panel.add(buttonAddStaff);
		...
		model.setColumnIdentifiers(Staff.columnNames);
		...
		table.setModel(Staff.searchStaff("", "By Name"));
	...
	public void actionPerformed(ActionEvent ae) {
		...
		else if (ae.getSource().equals(buttonAddStaff)) {
			this.setVisible(false);
			new AddStaffActivity(this, Staff).setVisible(true);
		}
		else if (ae.getSource().equals(buttonCheck)) {
			table.setModel(Staff.searchStaff(keywordTF.getText().trim(), byWhatCB.getSelectedItem().toString()));
		}
		else {}
	}
	...
	private void jTable_ClickMouseClicked(MouseEvent evt) {                                          
	   ...
	   new ManageStaff(value1, this).setVisible(true);
    }
}